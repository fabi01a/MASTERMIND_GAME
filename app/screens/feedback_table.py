...
def _render_table_header(cfg):
        ...
        header_row = (
            term.bold +
            cfg["PIPE"] + f"{'Attempt':^{cfg['ATTEMPT_W']}}" +
            cfg["PIPE"] + f"{'Your Guess':^{cfg['GUESS_W']}}" +
            cfg["PIPE"] + f"{'Correct Digit':^{cfg['CORRECT_DIGITS_W']}}" +
            cfg["PIPE"] + f"{'Correct Digit & Correct Position':^{cfg['CORRECT_POS_W']}}" +
            cfg["PIPE"]
        )
...
def _render_row(cfg, attempt_num, guess, feedback):
    ...
    row = (
        cfg["PIPE"]  + f"{attempt_num:^{cfg['ATTEMPT_W']}}" +
        cfg["PIPE"]  + f"{guess_str:^{cfg['GUESS_W']}}" +
        cfg["PIPE"]  + f"{feedback['correct_numbers']:^{cfg['CORRECT_DIGITS_W']}}" +
        cfg["PIPE"]  + f"{feedback['correct_positions']:^{cfg['CORRECT_POS_W']}}" +
        cfg["PIPE"] 
    )
